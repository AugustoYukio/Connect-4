...
function nextTurn(playerToTakeTurnNow, playerToTakeTurnAfter, previousColumn) {
  ...
  advanceTurn();
  console.log(
    "Turn " + moves + ", " + playerToTakeTurnNow.chipColor + "'s turn."
  );
...
function tryTurn(playerToTakeTurnNow, playerToTakeTurnAfter, previousColumn) {
  ...
  playerToTakeTurnNow.takeTurn(mainBoard, previousColumn, function(
    columnToDropIn,
    shouldAnimate
  ) {
    ...
    var chipWasDropped = helperMethods.dropChip(
      mainBoard,
      columnToDropIn,
      playerToTakeTurnNow.chipColor,
      function(column, j, chipColor) {
  ...
  for (var i = 1; i < 8; i++) {
    for (var j = 6; j > 0; j--) {
      ...
      if (mainBoard[i][j] == color) {
        if (
          i < 5 &&
          color == mainBoard[i + 1][j] &&
          color == mainBoard[i + 2][j] &&
          color == mainBoard[i + 3][j]
        )
          draw4Xs(i, j, "h");
        if (
          j < 4 &&
          color == mainBoard[i][j + 1] &&
          color == mainBoard[i][j + 2] &&
          color == mainBoard[i][j + 3]
        )
          draw4Xs(i, j, "v");
        if (
          i < 5 &&
          j > 3 &&
          color == mainBoard[i + 1][j - 1] &&
          color == mainBoard[i + 2][j - 2] &&
          color == mainBoard[i + 3][j - 3]
        )
          draw4Xs(i, j, "//");
        if (
          i < 5 &&
