...
def create_board(ctx_app, data):
    try:
        ...
        fail = fail_creation_board_schema.load(
            {'errors': [erro for erro in error.args], 'message': "Fail Creation Board"})
        return jsonify(fail), 301
    except BadRequest as error:
        fail = fail_creation_board_schema.load(
            {'error': [{'description': error.description}], 'message': "Fail Creation Board"})
...
def delete_board(ctx_app, board_id):
    ...
    if not board_id:
        return fail_delete_board_schema.load(
            {'message': 'Fail Delete', 'errors': [{1: f"Board id must not empty."}]}
        ), 404
...
def update_board(ctx_app, data):
    ...
    board = find_board(ctx_app, str(board_id))
    try:
        valid_data = validate_board_schema.dump(data)
    except ValidationError as error:
        return fail_update_board_schema.dump({'errors': [erro for erro in error.args]}), 400

    board_update = board[0] | valid_data
    try:
        affected_rows = ctx_app.db.session.execute(
            update(Board).where(Board.id.is_(board[0].get('id'))).values(**valid_data))
        if affected_rows.rowcount == 0:
            return fail_update_board_schema.load({'errors': [{1: f"Board id: {board_id} not found"}]}), 404
    except (IntegrityError,) as error:
        return fail_update_board_schema.load(
            {'errors': [{n_erro + 1: erro} for n_erro, erro in enumerate(error.orig.args)]}), 400
    else:
        ctx_app.db.session.commit()
    return success_update_board_schema.dump(board_update), 202
...
def find_board(ctx_app, board_id: str):
    ...
    try:
        ...
        fail = fail_get_board_schema.load({'errors': [erro for erro in error.args], 'message': "Fail Creation"})
        return fail, 301
