...
onmessage = function(e) {
  ...
  if (sobriety) {
    maxDepth = Math.round(
      Math.log(3000000) / Math.log(possibleMoves(currentBoard).length + 1)
    );
  } else {
    maxDepth = Math.round(
      Math.log(1000000) / Math.log(possibleMoves(currentBoard).length + 1)
    );
  }
  ...
  if (sobriety || Math.random() < 0.5) {
    ...
    var column = possibleMoves(currentBoard)[
      Math.floor(Math.random() * possibleMoves(currentBoard).length)
    ];
  }
